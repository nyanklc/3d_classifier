...
class Classifier3D(nn.Module):
    ...
    def forward(self, x):
        ...
        top_down, _ = torch.max(x, axis=-1)
        front, _ = torch.max(x, axis=-2)
        side, _ = torch.max(x, axis=-3)

        flat_3d = torch.flatten(self.feature_extractor_3d(x), start_dim=1)
        flat_2d_1 = torch.flatten(self.feature_extractor_2d(top_down), start_dim=1)
        flat_2d_2 = torch.flatten(self.feature_extractor_2d(front), start_dim=1)
        flat_2d_3 = torch.flatten(self.feature_extractor_2d(side), start_dim=1)
